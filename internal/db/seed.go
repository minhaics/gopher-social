...
func generateUsers(num int) []*store.User {
	...
	for i := 0; i < num; i++ {
		user := &store.User{
			...
			IsActive: true, // Set giá trị IsActive là true
		}
		// Thêm mật khẩu cho user (băm mật khẩu)
		password := "defaultPassword" // Đây là mật khẩu mặc định bạn muốn sử dụng
		err := user.Password.Set(password)
		if err != nil {
			log.Println("Error setting password:", err)
			continue // Nếu có lỗi, bỏ qua user này
		}
		users[i] = user
	}
	return users
